...
struct flat {
  ll x, y, ind;
  bool operator<(flat &o) const {
    return x < o.x;
  }
};

ll isNeighbour(flat a, flat b) {
...
int main() {
  ...
  for(ll i = 1; i < n; i++) {
    for (auto& x : a[i]) {
      ll j = prev(upper_bound(all(a[i - 1]), x)) - a[i - 1].begin();
      while(j < sz(a[i - 1]) && isNeighbour(x, a[i - 1][j])) {
        unite(x.ind, a[i - 1][j].ind);
        j++;
      }
    }
  } v64 cnt(q);
