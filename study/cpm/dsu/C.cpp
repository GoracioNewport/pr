...
int main() {
  ...
  forn(i,n) {
    forn(j,n) {
      if (i < j) edges.pb({(abs(a[i].fi - a[j].fi) + abs(a[i].se - a[j].se)) * (k[i] + k[j]), {i, j}});
    }
  } forn(i,n) edges.pb({c[i], {i, n}});
  init(n + 1);
  sort(all(edges));
  ...
  for (auto& i : edges) {
    if (getP(i.se.fi) != getP(i.se.se)) {
      ...
      anal += i.fi;
      if (i.se.se == n) stations.pb(i.se.fi + 1);
      else roads.pb({i.se.fi + 1, i.se.se + 1});
    }
  } cout << anal << ln << sz(stations) << ln << stations << ln << sz(roads) << ln << roads << ln;
