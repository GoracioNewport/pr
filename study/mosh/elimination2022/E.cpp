...
int main() {
  ...
  freopen("/mnt/hdd/pr/study/mosh/elimination2022/e2.txt", "r", stdin);
//  freopen("/mnt/hdd/pr/study/mosh/elimination/a0ans.txt", "w", stdout);
  ...
  while(t--) {
    ...
    if (isStack && isQueue) cout << "both" << ln;
    if (isStack && !isQueue) cout << "stack" << ln;
    if (!isStack && isQueue) cout << "queue" << ln;
    if (!isStack && !isQueue) cout << "none" << ln;
  }
