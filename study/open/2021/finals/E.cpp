...
int main() {
  fastCin();
	...
	while(t--) {
		...
		for (ll i = 0; i < n; i++) {
			for (ll j = i + 1; j < n; j++) {
				lines.pb({abs(a[i].fi - a[j].fi) * abs(a[i].fi - a[j].fi), abs(a[i].se - a[j].se) * abs(a[i].se - a[j].se)});
			}
		} 
		...
		while(q--) {
			...
			ld ma = -INF;
			for (auto& i : lines) ma = max(ma, i.fi * alph * alph + i.se);

			cout << fixed << setprecision(20) << sqrt(ma) << ln;
		}
	}  
